...
; (function ($w) {
    ...
    jm.mind.prototype = {
        ...
        _move_node: function (node, beforeid, parentid, direction) {
            ...
            if (!!node && !!parentid) {
                ...
                if (node.parent.isroot) {
                    ...
                    el.classList.forEach(function(className){
                        className !== 'selected' && this.remove(className)
                    }, el.classList)
            ...
            // 抛事件
            const node_position = ['_first_', '_last_']
            let prevId = ''
            if(node_position.includes(beforeid)){
                if(beforeid === '_first_'){
                    // 移动到最前面
                    prevId = ''
                }else if(beforeid === '_last_'){
                    // 移动到最后面
                    const pNode = this.get_node(parentid)
                    prevId = pNode.children[pNode.children.length - 1].id
                }

            }else {
                const beforeNode = this.get_node_before(node)
                prevId = beforeNode ? beforeNode.id : ''
            }
            const handler = jm.current.options.shortcut.handles;
            typeof handler.node_moved_callback === 'function'
            ? handler.node_moved_callback(node, prevId, parentid, direction)
            : console.error('节点移动的事件处理不是一个函数');
            return node;
        },
    ...
    jm.util = {
        ...
        node: {
            is_first_level_heading: function(node){
                return node.parent && node.parent.isroot
            },
            is_leaf: function(node){
                return node.data.chapterType === 'model' || node.data.type === 'model_diagram' || node.data.chapterType === 'text'
            },
            is_model: function(node){
                return node.data.chapterType === 'model' || node.data.type === 'model_diagram'
            },
            is_text: function(node){
                return node.data.chapterType === 'text'
            },
            is_heading: function(node){
                return !node.isroot && node.data.chapterType === 'heading'
            },
        }
    };
    ...
    jm.prototype = {
        ...
        my_destroy: function(){
            this._reset();
            jsMind.current = null;
            var panel = $d.querySelector('.jsmind-inner');
            panel && panel.remove();
        },
        ...
        dblclick_handle: function (e) {
            ...
            if (is_node) {
                var nodeid = this.view.get_binded_nodeid(element);
                if (!!nodeid) {
                    const handler = this.options.shortcut.handles
                    const node  = this.get_node(nodeid)
                    if(jm.util.node.is_heading(node) || node.isroot){
                        this.begin_edit(nodeid);
                        this.menu.changeOperateMode('rename')
                    }else if(jm.util.node.is_model(node)){
                        typeof handler.model_dblclick === 'function' ? handler.model_dblclick(node) : console.error('建模双击不是一个函数，请在快捷键里配置事件处理')
                    }else if(jm.util.node.is_text(node)){
                        typeof handler.text_dblclick === 'function' ? handler.text_dblclick(node) : console.error('文本双击不是一个函数，请在快捷键里配置事件处理')
                    }
                }
            }
        },
    ...
    jm.layout_provider.prototype = {
        ...
        toggle_node: function (node) {
            if (node.isroot) {
                return node.children.forEach(n => {})
            }
            if (node.expanded) {
    ...
    jm.view_provider.prototype = {
        ...
        create_node_element: function (node, parent_node) {
            ...
            if (!!node.topic) {
                if (this.opts.support_html) {
                    ...
                    const num = node.data && node.data.serialNumber ? node.data.serialNumber + '.' : ''
                    // 修改自定义节点样式
                    const {is_first_level_heading, is_heading, is_model, is_text} = jm.util.node
                    let htmlStr = ''
                    if(is_text(node)){
                        htmlStr = `
                            <img src="static/img/jsmind/注释.svg" style="width: 16px;cursor: pointer;float: left;padding-right: 4px" />
                            <span data-drag="true">${node.topic}</span>
                        `
                        d.classList.add('text')
                    }
                    else if(is_model(node)){
                        htmlStr = `
                        <img src="static/img/jsmind/模型图.svg" style="width: 16px;cursor: pointer;float: left;padding-right: 4px" />
                        <span data-drag="true">${node.topic}</span>
                    `
                    d.classList.add('model')
                    }else {
                        htmlStr = `<span data-drag="true">${num}${node.topic}</span>`
                    }
                    $h(d, htmlStr);
                } else {
                    $t(d, node.topic);
            ...
            // tooltip设置title
            d.setAttribute('title', node.topic);
        ...
        update_node: function (node) {
            ...
            if (!!node.topic) {
                if (this.opts.support_html) {
                    const num = node.data && node.data.serialNumber ? node.data.serialNumber + '.' : ''
                    const htmlStr = `<span data-drag="true">${num}${node.topic}</span>`
                    $h(element, htmlStr); 
                    // 这里会清空菜单，在下面重新加上
                    this.jm.menu.addMenu(node);
                } else {
        ...
        edit_node_end: function () {
            if (this.editing_node != null) {
                ...
                typeof handler.editEnd === 'function' && handler.editEnd(this.jm.menu.operateMode, node)
            }
            this.e_panel.focus();
        },
        ...
        setZoom: function (zoom) {
            ...
            for (var i = 0; i < this.e_panel.children.length; i++) {
                // this.e_panel.children[i].style.zoom = zoom; // scale
                this.e_panel.children[i].style.transform = `scale(${zoom})`; // scale
                this.e_panel.children[i].style.transformOrigin = `left top`; // scale
            };
            this.show(true);
            return true;
        ...
        show_nodes: function () {
            ...
            for (var nodeid in nodes) {
                ...
                if (!node.isroot && node.children.length > 0) {
                    var len = node.children.length >= 100 ? '...' : node.children.length
                    expander_text = node.expanded ? '-' : len;
                ...
                this.jm.menu && this.jm.menu.addMenu(node)
            }
            //
            console.log(this);
